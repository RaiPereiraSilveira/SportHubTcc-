    ...
    if (!$erros) {
        if (!sh_tabela_existe($pdo, 'assinaturas')) {
            ...
            try {
                ...
                error_log('Erro ao registrar assinatura: ' . $e->getMessage());
...
<section class="section-sm">
    ...
    <?php if ($sucesso): ?>
        <!-- ═══════════ CONFIRMAÇÃO ═══════════ -->
        <div class="form-card" style="max-width:760px;margin-inline:auto">
            ...
            <div class="data-table-wrap">
                <table class="data">
                    <tbody>
                        <tr><td style="color:var(--muted)">Plano contratado</td><td><strong><?= e($sucesso['plano']) ?></strong></td></tr>
                        <tr><td style="color:var(--muted)">Valor anual</td><td><strong>R$ <?= e(sh_money($sucesso['valor'])) ?></strong></td></tr>
                        <tr><td style="color:var(--muted)">Teste gratuito até</td><td><?= e($sucesso['trial_fim']->format('d/m/Y')) ?></td></tr>
                        <tr><td style="color:var(--muted)">Vigência da assinatura</td><td>até <?= e($sucesso['vigencia_fim']->format('d/m/Y')) ?></td></tr>
                        <tr><td style="color:var(--muted)">Situação</td><td><span class="tag tag-amber"><i class="fas fa-hourglass-half"></i> Em teste</span></td></tr>
                    </tbody>
                </table>
            </div>

            <h3 class="mt-3">Próximos passos</h3>
            <ol class="mt-1" style="display:grid;gap:12px;counter-reset:li">
    ...
    <?php else: ?>
        <!-- ═══════════ FORMULÁRIO ═══════════ -->
        <div style="display:grid;gap:26px;align-items:start" class="assinar-layout">
            <style>
            ...
            <aside class="assinar-resumo">
                <div class="card" style="padding:26px">
                    ...
                    <h3 class="mt-1" style="font-size:1.4rem"><?= e($plano_escolhido['nome']) ?></h3>
                    <p class="small mt-1"><?= e($plano_escolhido['descricao']) ?></p>

                    <div class="plan-price" style="margin:20px 0 4px">
                        <span class="plan-currency">R$</span>
                        <span class="plan-value" style="font-size:2.3rem"><?= e(sh_money($plano_escolhido['preco_anual'])) ?></span>
                    </div>
                    ...
                    <ul class="plan-features" style="margin:0;padding:0;border:none">
                ...
                <div class="row mt-2" style="justify-content:center;gap:16px">
                    <?php foreach ($planos as $p): if ($p['slug'] === $slug) continue; ?>
                        <a href="<?= e(sh_url('assinar.php?plano=' . urlencode($p['slug']))) ?>" class="small" style="color:var(--ember);text-decoration:underline">
