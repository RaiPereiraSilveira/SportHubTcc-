...
$page_titles = [
    ...
    'chaveamento.php'          => 'Chaveamento de Mata-mata',
    'configuracao.php'         => 'Regras do Campeonato',
    'indicadores.php'          => 'Indicadores para a Direção',
    'ocorrencias.php'          => 'Registro Disciplinar',
    'cobrancas.php'            => 'Cobranças',
    'emails.php'               => 'Comunicações Enviadas',
    'contas_lgpd.php'          => 'Contas e Titulares',
    'galeria.php'              => 'Galeria das Partidas',
    'estatisticas.php'         => 'Estatísticas dos Atletas',
    'seguranca.php'            => 'Verificação em Duas Etapas',
    'trocar_senha.php'         => 'Trocar Senha',
];
...
$titulos_por_perfil = [
    ...
    'admin'   => ['galeria.php' => 'Galeria das Partidas'],
];
...
/* 96px porque é esse o tamanho em que ele aparece: 44px no cabeçalho, o
   dobro em tela retina. O arquivo de 1254px que estava aqui pesava 729 KB e
   era baixado em toda página do sistema. O de 512px continua existindo para
   o ícone do aplicativo instalado. */
$logo_path      = sh_asset('img/Logo-96.png');
...
$pendencias = ['credenciamentos' => 0, 'lgpd' => 0, 'mensagens' => 0, 'cobrancas' => 0, 'emails' => 0];
if ($tipo_usuario === 'admin' && isset($pdo)) {
    try {
        ...
        if (sh_tabela_existe($pdo, 'cobrancas')) {
            $pendencias['cobrancas'] = (int)$pdo->query(
                "SELECT COUNT(*) FROM cobrancas WHERE status = 'pendente' AND vencimento < CURDATE()"
            )->fetchColumn();
        }
        if (sh_tabela_existe($pdo, 'emails_enviados')) {
            $pendencias['emails'] = (int)$pdo->query(
                "SELECT COUNT(*) FROM emails_enviados WHERE status = 'falhou'
                  AND created_at > DATE_SUB(NOW(), INTERVAL 7 DAY)"
            )->fetchColumn();
        }
    } catch (PDOException $ex) {
        sh_log_excecao($ex, 'contar pendências');
    }
}
...
$nav_principal = [
    'admin' => [
        ...
        ['chaveamento.php', 'admin/chaveamento.php', 'fa-sitemap',       'Chaveamento'],
    ...
    'aluno' => [
        ...
        ['estatisticas.php', 'aluno/estatisticas.php', 'fa-ranking-star', 'Atletas'],
        ['galeria.php',      'aluno/galeria.php',      'fa-images',       'Mural'],
    ],
];
...
$nav_gestao = [
    ...
    ['contas_lgpd.php',           'admin/contas_lgpd.php',           'fa-user-slash',    'Contas e titulares', 0],
    ['ocorrencias.php',           'admin/ocorrencias.php',           'fa-file-circle-exclamation', 'Registro disciplinar', 0],
    ['galeria.php',               'admin/galeria.php',               'fa-images',        'Galeria',         0],
    ['indicadores.php',           'admin/indicadores.php',           'fa-chart-pie',     'Indicadores',     0],
    ['cobrancas.php',             'admin/cobrancas.php',             'fa-money-bill-wave', 'Cobranças',     $pendencias['cobrancas']],
    ['emails.php',                'admin/emails.php',                'fa-paper-plane',   'Comunicações',    $pendencias['emails']],
    ['configuracao.php',          'admin/configuracao.php',          'fa-sliders',       'Regras do campeonato', 0],
];

$itens = $nav_principal[$tipo_usuario] ?? $nav_principal['aluno'];

/* O menu da coordenação tem oito entradas de topo — sete telas mais o
   submenu "Gestão" — e elas somam 929px. Com o logotipo e o bloco de perfil,
   o cabeçalho precisava de 1368px, e o container tinha 1220: a lista passava
   POR CIMA do avatar em qualquer largura de desktop, não só nas estreitas.

   Esta classe liga o tratamento compacto só para quem tem menu longo. O
   aluno (seis entradas) e o árbitro (duas) continuam com o espaçamento
   original — apertar os três para resolver o problema de um seria pior. */
$nav_cheia = (count($itens) + ($tipo_usuario === 'admin' ? 1 : 0)) >= 7;
?>
<!DOCTYPE html>
...
<header class="main-header<?= $nav_cheia ? ' main-header--nav-cheia' : '' ?>">
                        ...
                        <?php if ($tipo_usuario === 'admin'): ?>
                            <li class="nav-item has-sub">
                                <button type="button" class="nav-link nav-sub-toggle" aria-haspopup="true" aria-expanded="false">
                                    ...
                                    <?php endif; ?>
                                    <i class="fas fa-chevron-down u-font-size-62rem"></i>
                                </button>
                                <div class="nav-sub">
            ...
            <div class="header-right">
                <div class="user-menu">
                    ...
                    <div class="user-dropdown" id="userDropdown">
                        ...
                        <a href="<?= e(sh_url('seguranca.php')) ?>" class="dropdown-item">
                            <i class="fas fa-mobile-screen"></i><span>Verificação em duas etapas</span>
                        </a>
                        <a href="<?= e(sh_url('trocar_senha.php')) ?>" class="dropdown-item">
...
<div class="mobile-menu" id="mobileMenu">
    <nav aria-label="Navegação móvel">
        <ul class="mobile-nav-list">
            ...
            <li class="mobile-nav-item"><a href="<?= e(sh_url('seguranca.php')) ?>" class="mobile-nav-link"><i class="fas fa-mobile-screen"></i><span>Duas etapas</span></a></li>
