...
if ($tem_assinaturas) {
    try {
        $assinaturas = $pdo->query(
            "SELECT a.*, p.nome AS plano_nome, e.nome AS escola_nome, e.cidade, e.uf
             FROM assinaturas a
        ...
        foreach ($assinaturas as $a) {
            $resumo['total']++;
            if ($a['status'] === 'ativa') { $resumo['ativa']++; $resumo['receita'] += (float)$a['valor']; }
            if ($a['status'] === 'trial') { $resumo['trial']++; }
        }
    } catch (PDOException $ex) {
        error_log('Erro ao listar assinaturas: ' . $ex->getMessage());
        $erro = 'Erro ao carregar as assinaturas.';
    }
}
...
<div class="container">
    ...
    <div class="dashboard-stats">
        ...
        <div class="stat-card">
            <div class="stat-number" style="font-size:1.6rem">R$ <?= e(sh_money($resumo['receita'])) ?></div>
    ...
    <div class="admin-panel">
        ...
        <div class="tab-content active" id="tab-assinaturas">
            <?php if (!$assinaturas): ?>
                <div class="no-data"><i class="fas fa-file-invoice-dollar no-games-icon"></i> Nenhuma contratação registrada ainda.</div>
            <?php else: ?>
                <table>
                    <thead>
                        <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="POST" class="inline-form">
                                    ...
                                    <select name="status" onchange="this.form.submit()" style="max-width:150px">
        ...
        <div class="tab-content" id="tab-mensagens">
            ...
            <?php else: ?>
                <div class="games-list">
                    ...
                    <article class="game-card" style="<?= $m['lido'] ? '' : 'border-color:var(--ember)' ?>">
                        <div class="game-header">
                            <div>
                                <span class="game-sport"><?= e($m['nome']) ?></span>
                                <div class="detail-label" style="margin-top:4px"><?= e($m['email']) ?><?= $m['telefone'] ? ' · ' . e($m['telefone']) : '' ?></div>
                            </div>
                            <span class="status-badge <?= $m['lido'] ? 'finalizado' : 'em_andamento' ?>">
                                <?= e($assuntos[$m['assunto']] ?? $m['assunto']) ?>
                            </span>
                        </div>
                        <p class="observations" style="margin-top:14px"><?= nl2br(e($m['mensagem'])) ?></p>
